...
print(filter(team.data, TRB == max(TRB))['Teams',])
...
water.pkm <- filter(pokemon, Type.1 == "Water" | Type.2 == "Water")
...
# the table doesnt change how the data looks (apart fro listing how it's grouped)
...
# within each group.
summarize(type_2_groups,mean_Speed = mean(Speed),max_HP = max(HP))
...
summarize(group_by(filter(pokemon, Legendary == "False"), Generation), 
